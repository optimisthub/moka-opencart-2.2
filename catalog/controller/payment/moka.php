...
class ControllerPaymentMoka extends Controller
{
    private $errors = array();

    public function index()
    {
        $this->load->language('payment/moka');
        $this->load->model('payment/moka');

        $data = array();

        $data['config_moka_api_environment'] = $this->config->get('moka_api_environment');
        $data['checkout'] = html_entity_decode($this->url->link('payment/moka/checkout', '', true), ENT_COMPAT, 'UTF-8');
        $data['installment'] = html_entity_decode($this->url->link('payment/moka/installment', '', true), ENT_COMPAT, 'UTF-8');
        ...
        return $this->load->view('payment/moka', $data);
    }
    ...
    public function checkout()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $callback_url = html_entity_decode($this->url->link('payment/moka/callback', '', true), ENT_COMPAT, "UTF-8");
    ...
    public function installment()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateInstallment()) { 
            ...
            $data['html'] = $this->load->view('payment/moka_installment', $installmentDetails);
        ...
        if (isset($this->request->post['hashValue'])) {
            ...
            if ($code_for_hash == $hashValue) {
                ...
                if ($paymentDetail_result_code == 'Success') {
                    $this->model_payment_moka->addTransaciton($paymentDetail_data->PaymentDetail, $order_id);
                }
